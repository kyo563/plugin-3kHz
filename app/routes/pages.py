...
def settings_page(request: Request):
    if request.app.state.onecomme_mode:
        ...
        html = html.replace('</main>', '</div><div id="settings-bot-panel" role="tabpanel" aria-labelledby="settings-bot-tab" hidden>' + bot + '</div></main>')
        html = html.replace('</head>', '<link rel="stylesheet" href="/static/bot.css"><link rel="stylesheet" href="/static/settings-tabs.css"></head>')
        html = html.replace('</body>', '<script src="/static/settings-tabs.js"></script><script src="/static/bot.js"></script></body>')
