...
`/settings` は実用化済みのMVP設定画面です。

操作可能:
- `show_declared_player_name_on_overlay`（OBSに申告プレイヤーネームを表示するか）
...
- グループ人数: 3人固定
- 40秒状態変更ロック: 有効
- 参加回数管理: move_next時に現在対戦中グループへ +1

参加希望ワード編集、参加辞退ワード編集、判定方式変更は将来検討とします。
...
- 参加回数管理
  ...
  - userKey生値は保存しない
- `/settings` 実用化（現行MVP設定の確認・`show_declared_player_name_on_overlay` 切替）
- OBS表示の省略・視認性調整
  - 長い表示名は1行で省略表示
  - プレースホルダー「参加者募集中」は控えめに表示
  - `/overlay` は表示専用で操作UIなし
  - `/api/overlay-state` は内部情報を出さない最小レスポンスを維持
