...
## 視聴者向けコメント操作
- 参加希望: `参加希望` を含むコメント。
- 申告名付き参加希望: `参加希望 名前 たなかたろう`（`名前` の後ろを申告名として扱います）。
- 管理画面では `YouTube名（申告名）` で表示します。OBSは設定ON時のみ申告名を表示します。
- 参加辞退: `参加辞退` または `参加を辞退` を含むコメント。
- `キャンセル` / `取消` / `やめます` / `cancel` は辞退トリガーではありません。
- 次は参加希望トリガーではありません: `参加希望者` `参加希望順` `参加` `さんか` `join` `参加します` `参加したい` `キャンセル` `取消`
