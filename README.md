...
Windows 11 x64向けの未署名検証版です。Chrome・Python・OBS追加プラグインは利用者には不要です。Microsoft Edge WebView2 Runtimeを使用します。
...
**YouTubeコメントの自動取得、特定チャット製品との接続、コメントBotは同梱していません。** 手動管理から利用できます。コメントによる参加には、[外部入力仕様](docs/EXTERNAL_INPUT.md) に従った別途連携が必要です。Bot開発は保留中です。
