...
- 参加受付終了、重複、40秒の状態変更ロックは従来どおり。受信件数やコマンド件数は参加人数ではありません。
...
## 検証範囲
モックHTTPによる履歴除外、参加・指定名・辞退、重複、受付終了、通信再試行、APIエラー、停止中通信のキャンセル、管理キー権限を自動検証。実際のAPIキーとライブ配信を使った動作確認は利用者の設定後に必要です。公開済み1.0.0は変更していません。
